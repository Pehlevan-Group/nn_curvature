"""
training cifar in the contrastive learning context (Barlow Twins)
"""
...
from data import random_samples_by_targets, cifar10_contrastive, BARLOW_NORMLIZATION
...
def main():
    ...
    save_model_id = (
        fileid("barlow_cifar10_plane", args)
    ...
    if args.data == "cifar10":
        _, unaugmented_train_dataset, test_dataset = cifar10_contrastive(
            paths["data_dir"],
    ...
    for i in args.save_epochs:
        print(f'--- epoch {i} ---')
        model.load_state_dict(torch.load(os.path.join(load_model_dir, f'barlow_model_state_dict_e{i}.pt'), map_location='cpu'))
        model.eval()
