"""
compute the geometric quantities of ResNet by layers

Each intermediate layer is appended a avgpool2d to make things comparable.
"""
...
def main():
    ...
    save_model_id = (
        fileid("cifar10_deep", args) + f"_{args.target_digits[0]}_{args.target_digits[1]}"
    )
    ...
    for i, e in enumerate(args.save_epochs):
        ...
        for layer_idx, feature_map in enumerate(feature_maps):
            ...
            # intialize empty arrays to populate
            if i == 0:
                effective_volume = torch.zeros(frames + 1, num_samples, device=device)
            # otherwise read from save ones
            else:
