...
# load file
from vis import makepic_digits_plane_deep
...
# select plotter
makepic = makepic_digits_plane_deep
...
def main():
    ...
    entropy_all = torch.load(os.path.join(
        model_dir, "entropy.pt"), map_location=device)
    point_one = torch.load(os.path.join(
        model_dir, "point_one.pt"), map_location=device)
    point_two = torch.load(os.path.join(
        model_dir, "point_two.pt"), map_location=device)
    point_three = torch.load(
        os.path.join(model_dir, "point_three.pt"), map_location=device
    )
    origin = torch.load(os.path.join(
        model_dir, "origin.pt"), map_location=device)
    ...
    for i, (e, entropy, prediction) in enumerate(zip(
        args.save_epochs, entropy_all, predictions
    )):
        # load layers
        eff_vols_by_layer = []
        for layer_idx in range(1, 5):
            eff_vols_by_layer.append(torch.load(os.path.join(
                model_dir, f"eff_vol_layer{layer_idx}.pt"), map_location=device)[i]
            )
        ...
        # save
        save_name = (
            f"effvols_{model_id}" + f"_ternary" *
            args.ternary + f"_e{e}" + ".pdf"
        )
        plt.savefig(
